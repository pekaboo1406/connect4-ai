...
class FourConnect:
    ...
    def _FindMyopicMoves(self):
        ...
        for action in range(7):
            ...
            if row==-1:
                continue
            else: #Check if AI is winning 
                if row>0:
                    win=self._CanGameTreePlayerWin(row-1,action)  #Checking just one move of the AI player, assuming he puts his coin above mine, same col
                if win==True:
#                    print("Action {0} is a losing action because in the next move GameTree Player will win".format(action))
            ...
            #Check if I am winning with the row,col pair
            win=self._CanMyopicPlayerWin(row,action)
            if win==True:
#                print("Myopic can win : ",action,sep=" ")
                myopicWinAction=action
            #Check if AI wins at that row,col pair. If so I want to prevent him from winning, so I will block his winning position
...
#        print(myopicWinAction, gameTreeWinAction, validAction, losingAction)
        ...
        #if assert condition is false, assertion error message raised
